...
export class Day04 extends Component {
  render() {
    let solution1: ReactElement = (
      <div className="solution">
        <p>No valid passports found</p>
      </div>
    );

    let adventDay4: AdventOfCodeDay4 = new AdventOfCodeDay4();
    let validPassports: number = adventDay4.solvePart1();
    if (validPassports > 0) {
      solution1 = (
        <div className="solution">
          <p>Number of valid passports encountered: {validPassports}</p>
        </div>
      );
    }
    ...
    return (
      <div>
        <div className="section">
          <div className="section-header">Day 4</div>
          {solution1}
        </div>
      </div>
    );
...
export class AdventOfCodeDay4 {
  ...
  public solvePart1(): number {
    ...
    // Compile the data into a set of data objects.
    let parsedPassports: PassportData[] = [];

    let parts: string[] = this.passportData.split('-');

    for (let index = 0; index < parts.length; index++) {
      let element: string = parts[index].trim();

      let passportString: string = '';
      while (element.length > 0) {
        ...
        element = parts[index].trim();
      }

      // console.log("passport data: " + passportString);

      let passport: PassportData = this.parsePassportData(passportString);

      parsedPassports.push(passport);
    }

    console.log('passport count: ' + parsedPassports.length);
  ...
  private parsePassportData(data: string): PassportData {
    let passport: PassportData = new PassportData();

    let parts: string[] = data.trim().split(' ');

    parts.forEach((element: string) => {
      let splitElement: string[] = element.split(':');
      let key: string = splitElement[0];
      let value: string = splitElement[1];
  ...
  private isPassportValid(passport: PassportData): boolean {
    const validEyeColors: string[] = [ 'amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth' ];

    if (null == passport.byr) {
      return false;
    } else {
      ...
      let byrNumber: number = parseInt(passport.byr);
      if (isNaN(byrNumber)) {
        return false;
      }
    ...
    // cid is optional and does not require validation
    if (null == passport.cid) {
    }

    if (null == passport.ecl) {
      return false;
    } else {
    ...
    if (null == passport.eyr) {
      ...
      let eyrNumber: number = parseInt(passport.eyr);
      if (isNaN(eyrNumber)) {
        return false;
      }
    ...
    if (null == passport.hcl) {
      return false;
    } else {
      // hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
      let regEx: RegExp = new RegExp('^#([0-9A-Fa-f]{6})$');
      if (!regEx.test(passport.hcl)) {
        return false;
      }
    }

    if (null == passport.hgt) {
      return false;
    } else {
      // hgt (Height) - a number followed by either cm or in:
      let regEx: RegExp = new RegExp('^[0-9]+(in|cm)$');
      if (!regEx.test(passport.hgt)) {
        return false;
      }

      let heightValueString: string = passport.hgt.substr(0, passport.hgt.length - 2);

      let heightValue: number = parseInt(heightValueString);
    ...
    if (null == passport.iyr) {
      ...
      let eyrNumber: number = parseInt(passport.iyr);
      if (isNaN(eyrNumber)) {
        return false;
      }

      if (eyrNumber < 2010 || eyrNumber > 2020) {
        return false;
      }
    }

    if (null == passport.pid) {
      return false;
    } else {
      // pid (Passport ID) - a nine-digit number, including leading zeroes.
      let regEx: RegExp = new RegExp('^([0-9]{9})$');
      if (!regEx.test(passport.pid)) {
        return false;
      }
    }
