...
export class Day06 extends Component {
  render () {
    let advent: AdventOfCodeDay6 = new AdventOfCodeDay6();
    ...
    let groups: Group[] = advent.solve();

    // Total up the answers
    let affirmativeCount: number = 0;
    let unanimousCount: number = 0;
    groups.forEach((group: Group) => {
      affirmativeCount += group.answers.size;

      unanimousCount += group.unanimousCount;
    });

    solution = (
      <div className="solution">
        <p>Group data:</p>
        <p>Total affirmative answers: {affirmativeCount}</p>
        <p>Total unanimous answers: {unanimousCount}</p>
        <div>
          {groups.map((element: Group, index: number) => (
            <p key={index}>
              {JSON.stringify(element.affirmativeAnswers)}
              <br />
              Member count: {element.memberCount}
              <br />
              Unanimous count: {element.unanimousCount}
              <br />
            </p>
          ))}
        </div>
    ...
    return (
      <div>
        <div className="section">
          <div className="section-header">Day 6</div>
          {solution}
        </div>
      </div>
    );
...
export class AdventOfCodeDay6 {
  ...
  public solve (): Group[] {
    let groups: Group[] = Group.parseGroups(this.dataActual);
...
export class Group {
  public affirmativeAnswers: string[] = [];
  public memberCount: number = 0;
  public answers: Map<string, number> = new Map();
  public unanimousCount: number = 0;

  public static parseGroups (groupData: string[]): Group[] {
    let groups: Group[] = [];

    let group: Group = new Group();

    for (let index = 0; index < groupData.length; index++) {
      let element: string = groupData[index];

      // If the element is a blank line, that's the end of the group. Push the group and skip to the next element.
      if (0 === element.length) {
        group.finalize();
        groups.push(group);
      ...
      group.affirmativeAnswers.push(element);
    }

    if (group.affirmativeAnswers.length > 0) {
      group.finalize();
      groups.push(group);
    }
  ...
  private finalize () {
    this.memberCount = this.affirmativeAnswers.length;

    this.affirmativeAnswers.forEach((answer: string) => {
      for (let index = 0; index < answer.length; index++) {
        let char: string = answer[index];
        if (!this.answers.has(char)) {
          this.answers.set(char, 1);
        }
        else {
          this.answers.set(char, this.answers.get(char) + 1);
        }
      }
    });

    // Enumerate the answers to see which, if any, are unanimous
    this.answers.forEach((value: number, key: string, map) => {
      if (value === this.memberCount) {
        this.unanimousCount++;
      }
    });
  }
