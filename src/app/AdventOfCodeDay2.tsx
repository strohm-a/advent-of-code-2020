export class AdventOfCodeDay2 {
  ...
  public solvePart1(): string[] {
    ...
    for (let index = 0; index < this.passwords.length; index++) {
      const current = this.passwords[index];

      let parts: string[] = current.split(':');

      let counts: string[] = parts[0].split('-');
      let countLow: number = parseInt(counts[0]);

      let countHigh: number = parseInt(counts[1].split(' ')[0]);

      let countCharacter: string = counts[1].split(' ')[1];

      let password: string = parts[1].trim();

      // Count the number of the specified characters in the password.
      let characterCount: number = 0;
      let tempPassword: string = password;
      while (tempPassword.length > 0) {
        let char: string = tempPassword.substr(0, 1);
        if (char === countCharacter) {
          characterCount++;
        }

        tempPassword = tempPassword.slice(1);
      }

      // See if the password contains the required minimum and maximum number of the specified character.
      if (characterCount >= countLow && characterCount <= countHigh) {
  ...
  public solvePart2(): string[] {
    ...
    for (let index = 0; index < this.passwords.length; index++) {
      ...
      let positions: string[] = parts[0].split('-');
      let positionOne: number = parseInt(positions[0]);

      let positionTwo: number = parseInt(positions[1].split(' ')[0]);

      let countCharacter: string = positions[1].split(' ')[1];

      let password: string = parts[1].trim();

      // Convert the string into an array
      let array: any[] = password.trim().split('');

      //  Character offsets are one-based
      positionOne--;
      positionTwo--;

      /* 
      The specified character can only exist in the first position or the second postion, but not 
      both.
       */
      let isValid: boolean = false;

      // See if the character at the first position is the specified character.
      if (countCharacter === array[positionOne]) {
        isValid = true;
      ...
      // See if the character at the high position is the specified character.
      if (isValid) {
        result.push(password);
        continue;
      }
